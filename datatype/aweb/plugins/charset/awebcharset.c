...
static void Convertandwrite(STRPTR data, long datalen, struct Filterdata *fd, void *handle)
{
   ...
   dst = CodesetsConvertStr(CSA_SourceCodeset, fd->srcCodeset,
                            CSA_DestCodeset,   fd->dstCodeset,
                            CSA_Source,        data,
      ...
      if(CodesetsBase)
      {
         if(fd->first)
         {
            ...
            if(!found && xml)
            {
               ...
               {
                  strncpy(metaname,XMLDEFAULT,sizeof(XMLDEFAULT));
               }
            }
            if(found)
            {
